...
実装: `solvers/extremal_optimization.rs`

各構成要素（= 頂点）に**適応度**を与え、適応度の低い要素をべき乗則確率で
選んで変更する。SA と違い「悪い要素を狙い撃ち」する点が特徴。

**適応度**: 近傍 $i$ の適応度を、その近傍へ移ったときのスコア変化で定義する。

$$
\phi_i = \tilde S(x^{(i)}) - \tilde S(x)
$$

$\phi_i$ が小さい（= 大きく改善する）ほど「悪い適応度の要素」とみなし、
昇順ランク付けする（ランク 0 = 最も改善幅の大きい要素）。

**べき乗則選択**: ランク $k \in \{1, \dots, n\}$ を確率

$$
P(k) \propto k^{-\tau}
$$

で選ぶ。累積分布 $\mathrm{CDF}(k) = \dfrac{\sum_{j=1}^{k} j^{-\tau}}{\sum_{j=1}^{n} j^{-\tau}}$
を事前計算し、$u \sim \mathrm{Uniform}(0,1)$ を二分探索してランクを決める。
選ばれたランクの近傍へ**常に移動**する（受理判定なし）。
...
$\tau$ が大きいほどランク 0（最悪要素）に集中し、小さいほど一様に近づく。
...
GUI（`bin/gui.rs`）が回す実験のバックボーン。**プリセット条件で大量の SA を
...
- **SA 実行**: `execute(..., seed)` の `seed` が SA ループの乱数列を決める。
  ...
  `sm_seed = seed + 0xDEAD_BEEF` を使い、SA 本体の乱数列と干渉しない。
