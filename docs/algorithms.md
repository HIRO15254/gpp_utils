...
を事前計算し、$u \sim \mathrm{Uniform}(0,1)$ からランクを決める。$\lambda$ が同値の頂点
（$g/\deg$ は tie が出やすい）は[平均化規則](#533-同率適応度の扱い平均化規則全-eo-共通)で
扱う。引いたランクの頂点 $v_1$ を取り、**$v_1$ と反対集合の頂点が当たるまで $k_2$ を引き直して**
...
参照: 同率（tie）の扱いは全 EO 共通の[平均化規則](#533-同率適応度の扱い平均化規則全-eo-共通)に従う。

#### 5.3.3 同率適応度の扱い（平均化規則・全 EO 共通）

実装: `run_executor.rs` の `select_eo_rank`（スワップ版 `run_eo`・フリップ近傍版
`run_eo_flip`・汎用トレイト版 `solvers/extremal_optimization.rs` が共有する唯一の抽選規則）。

適応度 $\lambda$ が同値の頂点は「**同率群**」を成す。$\lambda$ 昇順に並べたとき
位置 $[s, e)$ を占める同率群について:

$$
\text{群の合計重み} = \sum_{k=s}^{e-1} w_k,
\qquad
\text{各メンバーの重み} = \frac{1}{e-s}\sum_{k=s}^{e-1} w_k,
\qquad
w_k = \frac{(k+1)^{-\tau}}{Z}
$$
...
例（適応度が 1-2-2-2-5 位の並び）: 本来 2 位・3 位・4 位が持つ重み $w_2 + w_3 + w_4$ を
合計し、同率の 3 頂点へ $\frac{w_2+w_3+w_4}{3}$ ずつ等分する。1 位・5 位の重みは変わらない。

**性質**:

- 同率が無ければ各群のサイズが 1 なので、素の $P(k) \propto k^{-\tau}$ 抽選と厳密に一致する。
- 群内は等確率なので、安定ソートに由来する**頂点番号バイアスは生じない**。
- 実装は $[0,1)$ の一様乱数 1 draw を累積分布 $Z^{-1}\sum_{j \le k}(j+1)^{-\tau}$
  （`build_power_law_cdf`）に当てて同率ブロックを走査するだけで、
  $k^{-\tau}$ の再計算も prefix 配列の再構築も要らない。
- $\lambda$ の一致判定は f64 の bit 等値。

この規則は 2026-07-30 に**最終確定**したもので、モード切り替えは存在しない
（それ以前に併存していた `EoTieBreak::Index` / `EoTieBreak::CompetitionRandom` と
