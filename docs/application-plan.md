...
master `c0dd61f` からは、暫定解からの実評価ベイスン、計算予算のスイープ・条件別指定、ラウンド単位実行の3機能を現行APIへ移植する。マージは現行の計算実装を基礎にこの3機能を実装する選択的な統合とし、masterの履歴も保持する。削除済み機能、旧ランナー・保存形式、追加の組み込み適応度、EOの別の同点処理・抽選法などは取り込まない。既存の探索アルゴリズムと乱数系列を維持する。
...
配列による明示列挙を唯一のスイープ構文とする。式評価は導入しない。グループごとに「グラフ×近傍方式×ソルバー設定×予算×探索シード」を展開し、全グループを連結する。EOはtaus×fitnesses、HC・SAは手法パラメータ×smoothingを展開する。展開結果は条件ID・シード順に表示し、乱数と識別子は配列位置に依存させない。
...
追加フィールドの`best_basin=false`と空の`conditions`は保存時に省略する。従来のスカラー予算と追加計測なしの設定・条件ID・バッチIDは維持する。予算配列・条件グループも未展開のまま保存し、個々の実行条件にはスカラーの予算を一つだけ持たせる。
...
runs.tsvは各ジョブ1行で予算`max_steps`を含む条件、状態、最終・最良指標などを展開する。未開始・失敗ジョブも行を持ち、結果のない列は空欄とする。traces.tsvは完了ジョブの各計測点1行で、補完した現在・最良値、平滑化・現行解および暫定解のベイスン指標を出す。分割配列はTSVへ複製しない。保存された計測点のスコア内訳はJSONの分割参照から取得でき、途中の時間・累積カウンターは保存しない。--include-incomplete指定時だけ、完了結果がないジョブの最新部分結果を追加し、異なる試行を混ぜない。
