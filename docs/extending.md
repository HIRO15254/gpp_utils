...
新しい手法種別（`SolverSpec`のvariant）の追加も同じ扱いとする。既存variantのシリアライズと`pinned_versions`を変えなければ既存の条件IDは変わらないので、記録済みのbatch_id（例：`experiments_sa_eo/run_recovery_v1.py`の`STAGES`）と一致することを確認する。新手法に独自の版キーが必要な場合は、`pinned_versions`へ足さず、`fitness:*`と同じく使う条件でだけ必須にする（全体へ足すと保存済みの実験が読めなくなる）。凍結した参照実装の網羅的な`match`には、新variant用のコンパイル専用の分岐（`unreachable!`）だけを足し、既存variantの分岐は変えずに各READMEへ記録する。
