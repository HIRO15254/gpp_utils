...
- **既存実装の整理（第3章）**
  ...
  - `random_k_average`は、評価値を次の受理まで保持するため、pseudo-marginal MHになる。定常分布は $\pi_K(s)\propto \mathbb{E}_S[\exp(-\sum_{w\in S}H(s^{(w)})/(KT))]$ である。
    ...
    - K≥2では非自明な平滑化になる。高温側（Θ≳−0.5）では、軟らかさKTの近傍ソフトミンとして解釈できる。K→nで全近傍平均（温度の付け替え）に戻る。
    - 以上を小規模グラフの全列挙と長時間シミュレーションで確認した。別のエージェントによる独立レビューでも確かめた。
...
- **予備実験（第5章）**：ベースラインと同じ18グラフ・10^6ステップでプロトタイプを試走した（シード8個、幾何グラフは32個）。
  - HKSは、幾何グラフで暫定解ベイスン値をnoneより平均18.6%下げた（静的τ=2）。n=500・d=10では57.6→40.0、n=500・d=5では19.7→7.2である。ランダムグラフでは差が1%以内で、ほぼ中立だった。
  - NSMは平均では中立だった（幾何+0.3%）。
  - 既存の`random_k_average`（K=1）は予測どおり悪化した（ランダム+1.9%、幾何+16.4%）。受理率はnoneの4〜21%だった。
  - 次に`gpp`へ実装して試す第一候補はHKSである。
...
**数値確認**：n=9、10のランダムグラフで全状態を列挙し、上の等式がいずれも $10^{-13}$ 以下の誤差で成り立つことを確かめた（`theory/check_affine.py`、`theory/check_affine2.py`）。
...
- **K=1**：影の解は温度Tのボルツマン分布のままである。提案が3頂点の反転に近いため、低温で受理率が大きく下がる。最適なΘは高温側へずれ、最適点での値はnoneと同等か悪くなると予想する。予備実験では、受理率がnoneの4〜21%に下がり、ベイスン値は悪化した（第5.4節）。
...
- 静的な使い方：$\rho\in\{1,1/2,0\}$（0はエロージョン）、κ=1。
- ホモトピー：κを1から0へ下げる。
...
- 静的：$\tau\in\{0.5,1,2,4\}$（予備実験ではτ=2が最良だった）。スケジュール：τを4から半減させて0へ（最後の20%は元の問題）。
...
第5章の予備実験の結果を踏まえた順位である。

| 提案 | 期待と予備実験の結果 | 実装の手間 | 優先度 |
|---|---|---|---|
| B HKS | 幾何グラフで大きく改善（τ=2で平均−18.6%）。ランダムグラフではほぼ中立 | 固有分解と密な局所場 | 高 |
| A NSM | `random_k`の雑音のない置き換え（K=1より明確によい）。ただしnoneに対する平均の改善はなかった | バケットの実装（EOの索引に近い） | 中 |
| D エネルギー依存温度 | 焼きなまし効果の上限と対照 | $O(1)$、ベースラインAの結果が必要 | 中 |
| C レプリカ結合 | 広い解への誘導 | 重なりの保持、予算の定義 | 中〜低 |
| バランス緩和・ノイズ法 | 対照 | 小 | 低 |
...
- 実装は`experiments_sa_eo/smoothing_survey/pilot`（Rust）である。
- グラフはベースラインの18組で、`gpp`の保存グラフと辺リストまで一致することを確かめた。
- MA：Flip、alpha=0.05、$10^6$ ステップ、一様ランダムな初期分割。受理規則は`gpp`と同じである。
- 手法は次の9種である。
  - none
  - rk K=1、4：`gpp`の`random_k_average`と同じ法則の評価を $O(K)$ で実装したもの
  - NSM ρ=1、0.5、エロージョン（κ=1）
  - HKS τ=0.5、2（静的）
  - HKSスケジュール：τ=4から10万ステップごとに半減させ、最後の20万ステップは元の問題に戻す
- Θはベースライン格子から選んだ−1.0〜+1.2の0.1刻み23点、シードは0〜7である。
- 幾何グラフ9組だけは、7手法（rk K=4とNSM ρ=0.5を除く）について、Θ=−0.3〜+1.0でシード8〜31を追加し、計32シードにした。
- 指標は、$10^6$ ステップ時点の暫定解のベイスン値である。最適Θの選択による楽観的な偏りを避けるため、シードを半分に分け、片方で選んだΘをもう片方で評価して平均した（交差検証）。
- ベースラインとの違いは次のとおりで、値を`gpp`の結果と直接比べることはできない。
  - シード数とΘ格子
  - 計測点（0、10^3、10^4、10^5、10^6の5点）
  - ベイスンの同点処理（番号の小さい頂点を選ぶ）
  - 乱数系列

### 5.2 ランダムグラフ（8シード）

交差検証で選んだ最適Θでの暫定解ベイスン値（$10^6$ ステップ）である。

| グラフ | none | rk K=1 | rk K=4 | NSM ρ=1 | NSM ρ=0.5 | NSM エロージョン | HKS τ=0.5 | HKS τ=2 | HKS スケジュール |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| random n=124 d=5 | 60.0 | 61.2 | 60.0 | 60.0 | 60.0 | 60.0 | 60.2 | 60.4 | 60.1 |
| random n=124 d=10 | 174.8 | 174.8 | 175.7 | 174.8 | 179.3 | 174.8 | 177.0 | 175.4 | 175.0 |
| random n=124 d=20 | 444.8 | 446.8 | 449.8 | 446.7 | 446.7 | 444.8 | 447.0 | 447.5 | 444.8 |
| random n=250 d=5 | 115.9 | 122.8 | 118.0 | 116.2 | 116.3 | 116.8 | 116.9 | 117.9 | 116.4 |
| random n=250 d=10 | 346.6 | 349.5 | 347.1 | 346.0 | 345.5 | 345.3 | 345.1 | 347.8 | 346.0 |
| random n=250 d=20 | 860.1 | 865.5 | 868.0 | 860.6 | 860.5 | 860.1 | 861.1 | 863.5 | 859.7 |
| random n=500 d=5 | 257.2 | 268.9 | 262.1 | 256.0 | 259.1 | 258.6 | 256.2 | 259.4 | 256.0 |
| random n=500 d=10 | 725.2 | 737.9 | 726.4 | 725.4 | 723.1 | 724.2 | 724.9 | 733.8 | 727.4 |
| random n=500 d=20 | 1725.7 | 1740.9 | 1732.7 | 1732.6 | 1733.3 | 1725.1 | 1733.6 | 1746.5 | 1731.6 |
| **noneとの差の平均** | — | +1.9% | +0.8% | +0.1% | +0.4% | +0.1% | +0.3% | +0.8% | +0.1% |

ランダムグラフでは、どの平滑化もnoneとの差が2%以内で、改善は見られなかった。最も悪いのはrk K=1である。

### 5.3 幾何グラフ（32シード）

| グラフ | none | rk K=1 | NSM ρ=1 | NSM エロージョン | HKS τ=0.5 | HKS τ=2 | HKS スケジュール |
|---|---:|---:|---:|---:|---:|---:|---:|
| geometric n=124 d=5 | 6.1 | 7.7 | 6.1 | 6.1 | 6.0 | 6.0 | 6.0 |
| geometric n=124 d=10 | 27.4 | 27.9 | 28.3 | 27.9 | 27.1 | 27.1 | 27.2 |
| geometric n=124 d=20 | 96.2 | 97.5 | 96.2 | 96.2 | 96.2 | 96.2 | 96.2 |
| geometric n=250 d=5 | 10.8 | 14.9 | 11.9 | 11.0 | 6.1 | 4.9 | 5.4 |
| geometric n=250 d=10 | 40.5 | 43.2 | 41.0 | 41.1 | 40.1 | 39.8 | 40.2 |
| geometric n=250 d=20 | 137.9 | 141.5 | 133.8 | 139.7 | 133.8 | 134.6 | 133.8 |
| geometric n=500 d=5 | 22.3 | 31.4 | 20.7 | 22.3 | 15.5 | 7.2 | 8.8 |
| geometric n=500 d=10 | 57.6 | 73.2 | 55.4 | 56.8 | 50.2 | 40.0 | 45.3 |
| geometric n=500 d=20 | 151.7 | 156.5 | 154.8 | 147.5 | 141.8 | 139.7 | 141.0 |
| **noneとの差の平均** | — | +16.4% | +0.3% | +0.3% | −11.0% | −18.6% | −15.9% |

HKSの改善は、n=250以上のグラフで標準誤差よりはるかに大きい。全シードで選んだ最適Θでの平均±標準誤差（32シード）は次のとおりである。

| グラフ | none | HKS τ=2 | HKS スケジュール | NSM ρ=1 |
|---|---:|---:|---:|---:|
| geometric n=250 d=5 | 9.5±0.7 | 4.6±0.3 | 5.1±0.4 | 10.7±0.5 |
| geometric n=500 d=5 | 19.7±1.2 | 7.2±0.4 | 8.9±0.6 | 20.7±1.1 |
| geometric n=500 d=10 | 57.6±2.5 | 40.0±1.3 | 43.5±1.9 | 55.4±2.3 |
| geometric n=500 d=20 | 151.7±5.4 | 139.7±0.3 | 141.0±0.8 | 149.7±2.6 |

HKSはシード間のばらつきも大きく減らした（n=500・d=20で標準誤差5.4→0.3）。$10^5$ ステップ時点でも、noneとの差の平均はHKS τ=2で−22.0%、スケジュールで−26.7%だった。ただし、スケジュールの $10^5$ 時点はまだ最初の段階（τ=4）の中にある。

### 5.4 命題3の予測の確認（`random_k`）

- noneの最適Θで比べると、rk K=1の受理率はnoneの3.6〜5.9%（ランダムグラフ）、6〜21%（幾何グラフ）だった。影の解から見た提案がほぼ3頂点の反転になることと整合する。
- rk K=1の最適Θは、noneより0〜+0.3だけ高温側にずれた。
- ベイスン値はK=1で一貫して悪化した（ランダム+1.9%、幾何+16.4%）。K=4はその中間だった。

いずれも3.4節の予測どおりである。

### 5.5 1ステップの費用

1スレッドで $10^6$ ステップを1回走らせた時間である（Θ=+0.4、5回のベイスン計測を含む）。

| 手法 | random n=500 d=10 | geometric n=500 d=10 |
|---|---:|---:|
| none | 64 ms | 34 ms |
| rk K=1／K=4 | 162／175 ms | 92／120 ms |
| NSM ρ=1／エロージョン | 412／249 ms | 355／182 ms |
| HKS τ=0.5／τ=2／スケジュール | 279／359／280 ms | 92／71／101 ms |

- HKSの前処理（固有分解済みの行列から $W$ を作る処理）は、n=500で1行列あたり約70 msかかる。
- NSMのソフトミン版は、バケットの走査と指数関数の評価に時間がかかっている。和を差分で更新すれば短縮できる。
- ベースラインの1ジョブは約2.5 CPU秒で、大半はベイスン計測である。探索部分が数倍になっても、全体への影響は小さい。

### 5.6 解釈

- **HKS**
  - 幾何グラフ、特にn=500のグラフと、非連結な次数5のグラフで大きく改善した。ランダムグラフと小さな幾何グラフではほぼ中立だった。これは仮説B1を支持する。
  - 熱核の長距離結合が、ドメインの大きな組み替えを安くしている可能性がある。ただし機構は確かめていない。
  - $10^6$ ステップ時点では静的τ=2がスケジュールより良く、仮説B2（スケジュールの優位）は支持されなかった。
- **NSM**
  - 平均の改善はなく、グラフによって良くも悪くもなった。仮説A1・A2は支持されなかった。
  - NSMが除くのは距離3以内の短い障壁である。幾何グラフの主な障害は大域的な組み替えにあり、ランダムグラフではMAがすでに効率的だと考えられる。
  - rk K=1よりは明確によく、`random_k`を置き換える意味はある。
- **`random_k`**：命題3の予測どおり、平滑化としては逆効果だった。
...
| 順 | 系列 | 値 | ジョブ数 |
|---:|---|---|---:|
| 1 | HKS静的 | τ∈{0.5, 1, 2, 4}（Flip） | 186,624 |
| 2 | HKSスケジュール | τ：4から半減して0へ（Flip） | 46,656 |
| 3 | 焼きなましの対照 | HKSスケジュールと同じ実効温度の推移 | 46,656 |
| 4 | NSM | ρ∈{1, 1/2, 0}、κ=1（Flip） | 139,968 |

ジョブ数は、系列数×81温度×18グラフ×32シードである。

- 予備実験ではτ=2が最良で、τ=0.5より良かった。そのため、HKS静的の範囲は大きい側へ広げた。
- Swap近傍への拡張は、Flipでの結果を見てから決める。

ベースラインAの見積もり（163,584ジョブで113 CPU時間）から、1ジョブ約2.5 CPU秒（大半がベイスン計測）とすると、次のとおりになる。

- HKS静的：約130 CPU時間（12並列で約11時間）
- NSM：約97 CPU時間

まずシード0〜7でふるい分け、有望な系列だけ0〜31へ広げる。条件IDはシードを含まないので、後で足したシードの結果もそのまま使える。
...
- **予備実験**：`gpp`の外の自作実装である。シードは8個（幾何グラフの7手法は32個）、Θは23点（追加分は14点）で、計測は5点の暫定解ベイスンだけである。ベースラインの計測（56点、現行解のベイスンも測る、同点処理は乱数）とは異なる。HKSが幾何グラフで効く機構は確かめていない。
