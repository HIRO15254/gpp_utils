...
def thetas():
    """SA の温度 Θ = log10(T)。-1.50 〜 +0.80 の 0.05 刻み = 47 点。

    浮動小数の累積誤差を避けるため整数から作る。
    """
    return [round(v / 100.0, 2) for v in range(-150, 81, 5)]


def taus():
    """EO のべき乗則指数 τ。コア 1.00〜1.70 を 0.05 刻み、両裾は粗く = 20 点。"""
    core = [round(v / 100.0, 2) for v in range(100, 171, 5)]  # 15 点
    return [0.60, 0.80, 0.95] + core + [1.85, 2.00]
